...
class flatfields(admin.ModelAdmin):
    ...
    list_display = ('pk','author','type','date_sozd','cena_agenstv','adress','dom_numb','kvart_numb',
                    'domclick', 'latitude', 'longitude',)
    list_filter = ['contract','type','domclick','komnat','author']
    search_fields = ['pk']
    fields = ['status_obj','type','raion','kr_raion','cena_agenstv','adress','dom_numb','kvart_numb','etag',#,
              'etagnost','author','client_name','client_tel','prim','domclick', 'youtube',
              'contract', 'contract_number', 'contract_date_end', ]
